...
$(document).ready(function () {
    ...
    $('#fullpage').fullpage({
        ...
        navigation: true,
        navigationTooltips: ['Main', 'About', 'Skills', 'Skills', 'Portfolio', 'Contact', 'Contact'],
    ...
    $('.aboutMignon_imgbox')
        .on('init', function (e, slick) {
            $('.aboutMignon_num > .count').html('<em>' + parseInt(slick.currentSlide + 1) + '</em> / ' + slick.slideCount)
        })
        .slick({
            slidesToShow: 1,
            slidesToScroll: 1,
    ...
    function counterAnimationReset() {
        ...
        $('.aboutMignon_count .slick-list').css({ height: '12.2rem' })
    }
    function triggerSlickMethod(event) {
        $('.aboutMignon_imgbox').slick(event)
        $('.aboutMignon_count').slick(event)
    }
    $('.aboutMignon_imgbox').on('afterChange', function (event, slick, currentSlide, nextSlide) {
        $('.aboutMignon_num > .count').html('<em>' + parseInt(slick.currentSlide + 1) + '</em> / ' + slick.slideCount)
    })
    $('.aboutMignon_count').on('afterChange', function (event, slick, currentSlide, nextSlide) {
    ...
    function focusAnimation(idx) {
        ...
        setTimeout(function () {
            ...
            $('.aboutMignon_count .slick-track').css({ top: inFocustransform + 'px' })
...
function mainHeight() {
    ...
    $('.mainCnts_wrap').css({ height: $windowsHeight })
}
//硫붿씤鍮꾩���
