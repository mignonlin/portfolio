        ...
        drag: function (event, ui) {
            ...
            for (i = inst.snapElements.length - 1; i >= 0; i--) {
                ...
                if (o.snapMode !== 'inner') {
                    ...
                    if (ts) {
                        ui.position.top =
                            inst._convertPositionTo('relative', { top: t - inst.helperProportions.height, left: 0 }).top - inst.margins.top
                    }
                    if (bs) {
                    ...
                    if (ls) {
                        ui.position.left =
                            inst._convertPositionTo('relative', { top: 0, left: l - inst.helperProportions.width }).left - inst.margins.left
                    }
                    if (rs) {
                ...
                if (o.snapMode !== 'outer') {
                    ...
                    if (bs) {
                        ui.position.top =
                            inst._convertPositionTo('relative', { top: b - inst.helperProportions.height, left: 0 }).top - inst.margins.top
                    }
                    if (ls) {
                    ...
                    if (rs) {
                        ui.position.left =
                            inst._convertPositionTo('relative', { top: 0, left: r - inst.helperProportions.width }).left - inst.margins.left
                    }
                }
    ...
    $.ui.intersect = function (draggable, droppable, toleranceMode) {
        ...
        switch (toleranceMode) {
            ...
            case 'pointer':
                ...
                draggableTop =
                    (draggable.positionAbs || draggable.position.absolute).top + (draggable.clickOffset || draggable.offset.click).top
                return (
            ...
            for (row = 0; row < numMonths[0]; row++) {
                ...
                for (col = 0; col < numMonths[1]; col++) {
                    ...
                    calender +=
                        '</tbody></table>' +
                        (isMultiMonth
                            ? '</div>' + (numMonths[0] > 0 && col === numMonths[1] - 1 ? "<div class='ui-datepicker-row-break'></div>" : '')
                            : '')
                    group += calender
                }
                html += group
            ...
            if (!inst.yearshtml) {
                ...
                if (secondary || !changeYear) {
                    ...
                    for (; year <= endYear; year++) {
                        inst.yearshtml +=
                            "<option value='" + year + "'" + (year === drawYear ? " selected='selected'" : '') + '>' + year + '</option>'
                    }
